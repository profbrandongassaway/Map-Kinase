    ...
    function initializeSvg() {
            ...
            const draw = SVG().addTo('#svgCanvas').size(800, 600).viewbox(viewBox.x, viewBox.y, viewBox.width, viewBox.height);
            const snapOrderForBookmark = (protboxId = null) => {
              const pid = protboxId ? String(protboxId) : '';
              if (bookmarkKey === 'ks' || pid.startsWith('ks_')) {
                return ['W1','W2','E1','E2','N2','N1','N3','S2','S1','S3'];
              }
              return ['N2','N1','N3','S2','S1','S3','E1','E2','W1','W2'];
            };
            ...
            const pickFreeSnap = (protboxId, radius = ptmSnapRadius) => {
              ...
              // Preferred order (bookmark-specific sequence)
              const order = snapOrderForBookmark(protboxId);
            ...
            try {
                bookmarkKey = data && data._bookmark_key ? String(data._bookmark_key) : null;
                if (!bookmarkKey) {
                    const activeNav = document.querySelector('#bookmark_selector a.nav-link.active');
                    const navVal = activeNav ? activeNav.getAttribute('data-value') : null;
                    if (navVal) bookmarkKey = navVal;
                }
                const persistKey = bookmarkKey;
                if (persistKey) {
            ...
            try {
                ...
                if (data && data.kegg_bg_image && data._kegg_preview && data._kegg_preview.show) {
                    ...
                    try {
                        ...
                        console.log('m3: embedding/updating kegg bg image (canvas)', 800, 600, 'scale', scale, 'offsets', offx, offy, 'opacity', opacity);
                        ...
                        const applySizing = (element, naturalWidth, naturalHeight) => {
                            const baseWidth = Number(naturalWidth) || (fallbackSize ? fallbackSize.width : null) || Number(element.width()) || 800;
                            const baseHeight = Number(naturalHeight) || (fallbackSize ? fallbackSize.height : null) || Number(element.height()) || 600;
                        ...
                        if (imgEl) {
                            ...
                            imgEl.loaded(function(loader) {
                                ...
                                const naturalWidth = Number.isFinite(candidateWidth) && candidateWidth > 0 ? candidateWidth : (fallbackSize ? fallbackSize.width : Number(imgEl.width()) || 800);
                                const naturalHeight = Number.isFinite(candidateHeight) && candidateHeight > 0 ? candidateHeight : (fallbackSize ? fallbackSize.height : Number(imgEl.height()) || 600);
                                applySizing(imgEl, naturalWidth, naturalHeight);
            ...
            const applyPositionForEntry = (entry, which = 'before') => {
                ...
                if (entry.type === 'prot-box') {
                    ...
                    translateProtbox(entry.id, dx, dy, { rect, skipRectMove: false }, handleVisit);
                    propagateLinkedMove(entry.id, dx, dy, new Set([entry.id]), handleVisit);
                    cleanupBrokenLinks();
            ...
            const showTextBoxMenu = (evt, id, editor, isShape = false) => {
                ...
                ocNoneRadio.name = `outline_none_$<built-in function id>`;
...
function rebuildGroupIndexes() {
            ...
            function initializeGroupState() {
                ...
                Object.keys(groupMap).forEach(gid => {
                    ...
                    if (!groupMap[gid].protbox_ids.length) {
                        delete groupMap[gid];
                    }
                });
                if (Array.isArray(groups)) {
            ...
            function moveGroupMembers(groupId, deltaX, deltaY, options = {}) {
                ...
                members.forEach(m => {
                    ...
                    if (m.type === 'prot-box') {
                        const pid = normalizeProtboxId(m.id);
                        const rect = draw.findOne(`[data-id="${pid}"]`);
                        translateProtbox(pid, deltaX, deltaY, { rect, skipRectMove: false }, visitedHandles);
                        propagateLinkedMove(pid, deltaX, deltaY, new Set([pid]), visitedHandles);
                    ...
                    group.forEach(assocElement => {
                        ...
                        if (assocType === 'handle-line') {
                            ...
                            assocElement.element.plot(
                                parseFloat(assocElement.element.attr('x1') || 0) + deltaX,
                                parseFloat(assocElement.element.attr('y1') || 0) + deltaY,
                                parseFloat(assocElement.element.attr('x2') || 0) + deltaX,
                                parseFloat(assocElement.element.attr('y2') || 0) + deltaY
                            );
                        } else {
            ...
            const propagateLinkedMove = (anchorId, deltaX, deltaY, visited = new Set(), visitedHandles = new Set()) => {
                ...
                Object.keys(links).forEach(peerId => {
                    ...
                    translateProtbox(peerId, deltaX, deltaY, { rect }, visitedHandles);
                    propagateLinkedMove(peerId, deltaX, deltaY, visited, visitedHandles);
                });
            };
            const maybeSnapProtbox = (movingId, rectRef = null) => {
                ...
                if (extraDX || extraDY) {
                    const handleVisit = new Set();
                    translateProtbox(movingId, extraDX, extraDY, { rect, skipRectMove: false }, handleVisit);
                    propagateLinkedMove(movingId, extraDX, extraDY, new Set([movingId]), handleVisit);
                }
            ...
            const unsnapAndNudge = (protboxId) => {
                ...
                const mover = () => {
                    translateProtbox(protboxId, dx, dy, { rect });
                    propagateLinkedMove(protboxId, dx, dy);
                };
                if (mkHistory) {
            ...
            const moveSelectedElement = (deltaX, deltaY, pointerX = null, pointerY = null) => {
                ...
                if (selectionMap.size > 1) {
                    selectionMap.forEach(entry => {
                        ...
                        if (entry.type === 'group') {
                            ...
                            const handleVisit = new Set();
                            translateProtbox(entry.id, deltaX, deltaY, { rect: entry.element, skipRectMove: false }, handleVisit);
                            propagateLinkedMove(entry.id, deltaX, deltaY, new Set([entry.id]), handleVisit);
                            cleanupBrokenLinks();
                ...
                if (selectedType === 'prot-box' && selectedProtboxId) {
                    const handleVisit = new Set();
                    translateProtbox(selectedProtboxId, deltaX, deltaY, { rect: selectedElement, skipRectMove: false }, handleVisit);
                    propagateLinkedMove(selectedProtboxId, deltaX, deltaY, new Set([selectedProtboxId]), handleVisit);
                    cleanupBrokenLinks();
            ...
            const spawnPtmForProtbox = (protboxId, uniprot, ptmKey, options = {}) => {
                ...
                const overridePosKey = existingOverride?.ptm_position || null;
                const ptmPosKey = ptm?.ptm_position || null;
                const snapForKey = (key) => (key && snapPoints && snapPoints[key]) ? { ...snapPoints[key], key } : null;
                ...
                if (!snapshotSeed?.shape) {
                    if (fixedPosition && fixedPosition.x !== undefined && fixedPosition.y !== undefined) {
                        ...
                        const hasOverrideCoords = overrideX !== null && overrideY !== null;
                        if (!forceCenterSpawn && hasOverrideCoords) {
                            snapChoice = {
                                x: overrideX,
                                y: overrideY,
                                key: overridePosKey || preferredSnapKey || ptmPosKey || null
                            };
                        } else {
                            const keyedChoice = !forceCenterSpawn
                                ? (snapForKey(overridePosKey) || snapForKey(ptmPosKey) || snapForKey(preferredSnapKey))
                                : null;
                            if (keyedChoice) {
                                snapChoice = keyedChoice;
                ...
                if (snapshotSeed?.shape && snapshotSeed.shape.x !== undefined && snapshotSeed.shape.y !== undefined) {
                    ...
                    newX = snapChoice.x;
                    newY = snapChoice.y;
                    posKey = snapChoice.key || overridePosKey || ptmPosKey || preferredSnapKey || null;
            ...
            const autoSpawnPtmsForProtbox = (pb) => {
                ...
                const snapOrder = snapOrderForBookmark(pb.protbox_id);
                const snapPoints = protboxSnapPoints[pb.protbox_id] || {};
                proteins.forEach((uni) => {
                    const prot = proteinData && proteinData[uni];
                    ...
                    for (let i = 0; i < limit; i++) {
                        const [ptmKey, ptmVal] = entries[i];
                        const shapeId = `${uni}_${ptmKey}_shape`;
                        if (draw.find(`[data-id="${shapeId}"]`).length) {
                            continue;
                        }
                        const ptmPosKey = typeof ptmVal?.ptm_position === 'string' ? ptmVal.ptm_position : null;
                        const posKey = (ptmPosKey && snapPoints[ptmPosKey])
                            ? ptmPosKey
                            : snapOrder[idx % snapOrder.length];
                        idx += 1;
                        spawnPtmForProtbox(pb.protbox_id, uni, ptmKey, {
