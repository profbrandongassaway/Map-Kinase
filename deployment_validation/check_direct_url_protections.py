...
def _render_markdown_summary(base_url: str, results: list[ProbeResult]) -> str:
    """Render a markdown summary suitable for pasting into Section 6 of the plan."""
    ...
    lines: list[str] = []
    lines.append("# Map-Kinase Direct URL Protection Check")
    lines.append("")
    lines.append("Generated by `deployment_validation/check_direct_url_protections.py`.")
    lines.append("")
    lines.append(f"Base URL: `{base_url}`")
    lines.append(f"Checked at (UTC): {datetime.now(timezone.utc).isoformat()}")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("| Path | HTTP status | Result | Notes |")
    lines.append("| --- | --- | --- | --- |")
    for r in results:
        lines.append(
            f"| `{_esc(r.path)}` | {_esc(r.status)} | **{r.result}** | {_esc(r.notes)} |"
        )
    lines.append("")
    if counts.get("FAIL", 0):
        lines.append(
            "One or more probed paths returned content that looked directly served. "
            "Address the FAIL rows before sending the OIT response."
        )
    elif counts.get("REVIEW", 0):
        lines.append(
            "All probed paths returned protected statuses or redirected to the application; "
            "any REVIEW rows require manual confirmation that the response is the app route, "
            "not a directory listing or file."
        )
    else:
        lines.append("All probed paths returned protected statuses or safe redirects.")
    lines.append("")
    return "\n".join(lines)
...
def main(argv: list[str]) -> int:
    ...
    summary_path = results_dir / "url_check_summary.md"
    summary_path.write_text(_render_markdown_summary(base_url, results), encoding="utf-8")
